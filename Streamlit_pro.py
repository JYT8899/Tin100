...
Selvstendig = st.selectbox("Selvsendig drivende", ["Yes", "No"],
                           help="Hvordan driver henter du inntekt?", key=2)

Utdanning = st.selectbox("Utdanning", ["Yes", "No"],
                         help="Utdanning høgre enn vidergåande.", key=3)

Barn = st.number_input("Barn under 18 år", min_value=0, max_value=100, value=0, step=1, key=2)

Eiendom = st.selectbox("Eiendomsområde", ["Urban", "Semiurban", "Landlig"],
                       help="Hvilke eigendomsområder er du fra av de tre alternativene?", key=4)

Kredit_hist = st.number_input("Kredit historie", min_value=0.0, max_value=1.0, step=0.05, value=0.5,
                              help="Her snakker vi om kreditt verdien.", key=3)
